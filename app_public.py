...
def get_selected_training_ids(scores: dict) -> list[str]:
    _, check_to_training = load_training_data()
...
def generate_mobile_html_report(scores: dict) -> tuple[Path, Path]:
    ...
    training_link_url = training_menu_path.name
    ...
    if qr_src:
        ...
        qr_html = '<div class="qr-missing">QR画像を作成できませんでした</div>'
...
if st.button("レポート作成", type="primary"):
    try:
        ...
        if output_type in ["スマホ用HTML", "両方"]:
            ...
            st.info("HTMLレポートには自主トレ内容を表示せず、QRコード・ボタン・URLのみ表示します。")
