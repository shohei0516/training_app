...
def generate_mobile_html_report(scores: dict) -> Path:
    ...
    if total_score == 0:
        comment = "大きなフォーム上の問題は確認されませんでした。現在のフォームを維持しながら、継続的にコンディショニングを行いましょう。"
    elif total_score <= 6:
        comment = "一部に軽度のフォーム課題が確認されました。該当フェーズを中心に、安定性と再現性を高めるトレーニングを行いましょう。"
    else:
        comment = "複数のフェーズでフォーム課題が確認されました。投球動作全体の連動性を高めるため、段階的なトレーニングが推奨されます。"
    ...
    if problem_checks:
        for check in problem_checks:
            ...
            problem_html += f"""
            <div class="card">
                <div class="badge">{escape(check["phase"])}</div>
                <h3>{escape(cid)}　{escape(check["title"])}</h3>
                <p class="score">スコア：{score}</p>
                <p>{escape(check["criteria"][score]).replace(chr(10), "<br>")}</p>
            </div>
            """
    else:
        problem_html = """
...
<div class="container">
    ...
    <div class="card">
        <p>
            チェックされた項目に対応する自主トレーニングを実施してください。<br>
            1点・2点のどちらの場合も、該当IDに紐づくトレーニングをすべて出力する運用です。
        </p>
    </div>
