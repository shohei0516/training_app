...
def image_to_base64_src(path: Path) -> str:
    if not path.exists():
        return ""
...
def find_qr_image() -> Path | None:
    candidates = []

    for folder in [OUTPUT_DIR, BASE_DIR / "qr", ASSETS_DIR / "qr"]:
        if folder.exists():
    ...
    qr_like = [p for p in candidates if "qr" in p.name.lower()]
    if qr_like:
        return qr_like[0]
...
def generate_mobile_html_report(scores: dict) -> Path:
    ...
    for phase in phase_names:
        ...
        image_path = PHASE_IMAGE_DIR / image_name
        img_src = image_to_base64_src(image_path)

        star_html = '<div class="star">★</div>' if phase_problem_map[phase] else ""
        checks_text = " / ".join(phase_check_map[phase]) if phase_check_map[phase] else "チェックなし"
        problem_class = " phase-problem" if phase_problem_map[phase] else ""
        ...
        phase_cards_html += f"""
        <section class="phase-card{problem_class}">
    ...
    if problem_checks:
        for check in problem_checks:
            ...
            criteria_text = check["criteria"][score].replace("\n", "<br>")
    ...
    qr_path = find_qr_image()
    qr_src = image_to_base64_src(qr_path) if qr_path else ""

    if qr_src:
        qr_html = f'<img class="qr-img" src="{qr_src}" alt="QRコード">'
    else:
        qr_html = """
        <div class="qr-missing">
            QR画像なし<br>
            output または qr フォルダに<br>
            QR画像を保存してください
        </div>
        """
...
<div class="container">
    ...
    <h2 class="section-title">投球フェーズ</h2>
    {phase_cards_html}
    ...
    <section class="qr-card">
        {qr_html}
        <p>
            QRコードから、チェックされた項目に対応する<br>
            自主トレーニングシートを確認してください。
        </p>
...
if st.button("レポート作成", type="primary"):
    try:
        ...
        if output_type in ["スマホ用HTML", "両方"]:
            ...
            html_path = generate_mobile_html_report(scores)

            st.success("スマホ用HTMLレポート完成！")

            with open(html_path, "rb") as f:
                st.download_button(
                    label="スマホ用HTMLをダウンロード",
                    data=f,
