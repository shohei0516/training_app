# -*- coding: utf-8 -*-
"""
Pitching report generator for Streamlit public app.
"""
...
def extract_training(eval_df: pd.DataFrame, link_df: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = link_df.merge(problem_ids, on="チェックID", how="inner")
...
def build_ppt_report(
    ...
    if len(prs.slides) == 0:
        raise ValueError("PPTテンプレートにスライドがありません。")
...
def generate_report() -> dict:
    ...
    if not pdf_done:
        pdf_done = export_pdf_libreoffice(OUTPUT_PPTX, OUTPUT_DIR)

    generated_pdf = OUTPUT_PDF if OUTPUT_PDF.exists() else None
...
def main() -> None:
    ...
    print(f"推奨トレーニング数: {result['training_count']}")
